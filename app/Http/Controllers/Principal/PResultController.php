...
class PResultController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $teacherCollection = $teacherCollection->map(function ($teacher) {
            ...
            // PRE progress
            $preForms = collect(['pre' => $pre]);
            $preCompleted = $preForms->filter()->count();
            $preTotal = $preForms->count();
            $preProgress = $preTotal > 0 ? round(($preCompleted / $preTotal) * 100) : 0;
            ...
            $externalProgress = $externalTotal > 0 ? round(($externalCompleted / $externalTotal) * 100) : 0;
            ...
            $postProgress = $postTotal > 0 ? round(($postCompleted / $postTotal) * 100) : 0;

            // PRE status
            $preStatus = $pre ? 'Completed' : 'Pending';
            ...
            } elseif ($externalPdpc->result === 'PASS' && $externalFeedback) {
        ...
        if ($status !== 'all') {
            $teacherCollection = $teacherCollection
                ->filter(fn($teacher) => $teacher->evaluation_status === $status)
                ->values();
        }
    ...
    public function show($gn_id)
    {
        ...
        if ($externalPdpc) {
            $externalFeedback = DB::table('post_response')
                ...
                ->where('post_response.attempt_no', $externalPdpc->attempt_no)
                ->where('post_response.status', 'Submitted')
                ->select('post_response.*', 'post_form.form_name')
                ->orderByDesc('post_response.responseID')
                ->first();
        ...
        $externalHistory = $externalAttempts->skip(1)->values();
        ...
        // Overall progress
        $completedForms = $preCompleted + $externalCompleted + $postCompleted;
        $totalForms = $preTotal + $externalTotal + $postTotal;
        $overallProgress = $totalForms > 0 ? round(($completedForms / $totalForms) * 100) : 0;
        ...
        if (
            ...
            $evaluationStatus = 'Completed';
        } elseif ($externalPdpc && $externalPdpc->result === 'REPEAT') {
            $evaluationStatus = 'Repeat Required';
    ...
    public function pre($responseID)
    {
        ...
        return view('principal.result-pre', compact(
            'guru',
            'form',
            'response',
            'scores',
    ...
    public function pdpc($responseID)
    {
        ...
        return view('principal.result-pdpc', compact(
            'guru',
            'form',
            'response',
            'scores',
    ...
    public function post($responseID)
    {
        ...
        // Teacher must belong to principal school
        $guru = $this->getGuruNewForPrincipal($response->gn_id, $principal->schoolID);
        ...
        return view('principal.result-post', compact(
            'guru',
            'form',
            'response',
            'existingAnswers'
    ...
    private function getGuruNewForPrincipal($gnId, $schoolID)
    {
        $guru = DB::table('guru_new')
            ->where('gn_id', $gnId)
    ...
    private function attachFeedbackEvaluator($response): void
    {
        ...
        $response->evaluator_role = $response->observation_stage === 'EXTERNAL'
            ? 'External Observer'
            : 'Observer';
        ...
        if ($response->observation_stage === 'POST' && $response->observer_id) {
            ...
            $observer = DB::table('observer')
                ->where('observer_id', $response->observer_id)
                ->first();

            if ($observer) {
                $teacher = DB::table('teacher')
                    ->where('teacherID', $observer->teacherID)
                    ->first();
        ...
        if ($response->observation_stage === 'EXTERNAL' && $response->external_observer_id) {
            ...
            $externalObserver = DB::table('external_observer')
                ->where('external_observer_id', $response->external_observer_id)
                ->first();

            if ($externalObserver) {
                $teacher = DB::table('teacher')
                    ->where('teacherID', $externalObserver->teacherID)
                    ->first();
