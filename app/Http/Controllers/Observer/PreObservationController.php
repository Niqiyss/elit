...
class PreObservationController extends Controller
{
    ...
    public function store(Request $request, $gn_id)
    {
        ...
        // Use the exact version displayed to the observer
        ...
        DB::transaction(function () use ($request, $gn_id, $observer, $form) {
            ...
            $this->saveSectionComments($request, $form, $response);
    ...
    public function update(Request $request, $responseID)
    {
        ...
        DB::transaction(function () use ($request, $response, $form, $observer) {
            ...
            PreScore::where('responseID', $response->responseID)->delete();
            $this->saveScores($request, $form, $response);

            PreSectionComment::where('responseID', $response->responseID)->delete();
            $this->saveSectionComments($request, $form, $response);
    ...
    {
        $form = PreForm::where('status', 'Active')
            ->with([
    ...
    {
        return PreForm::where('formID', $formID)
            ->with([
                'sections' => fn($query) => $query->orderBy('display_order'),
                'sections.criteria' => fn($query) => $query->orderBy('display_order'),
            ])
